...
if os.path.isdir("/scratch/hasm"):
    ...
    #for full data
    data_file_path = os.path.join(scratch_dir, "Data", "Lesion", "ATLAS_R1.1_Protocols", "0_freesurfer-6.0.0_full_improved_symlink", "train_brainmask_AnatSpaceReoriented.h5") 
...
def train(fold, train_patient_indexes, val_patient_indexes):
    ...
    if not os.path.isdir(log_dir):
        os.mkdir(log_dir)
    num_slices_train = len(train_patient_indexes) * dim_z
    num_slices_val = len(val_patient_indexes) * dim_z
